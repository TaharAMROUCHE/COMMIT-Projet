<?php 
    ...
    if(!empty($_POST['prenom']) && !empty($_POST['nom'])&& !empty($_POST['email']) && !empty($_POST['conf_email']) &&  !empty($_POST['mdp']) && !empty($_POST['conf_mdp']))
    {
        ...
        $check = $bdd->prepare('SELECT prenom, nom, email, mdp FROM utilisateurs WHERE email = ?');
                    ...
                    if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                      if($email == $conf_email){
                        if($mdp == $conf_mdp){
                            ...
                            $insert = $bdd->prepare('INSERT INTO utilisateurs(prenom, nom, email, mdp, ip) VALUES(:prenom, :nom, :email, :mdp, :ip)');
